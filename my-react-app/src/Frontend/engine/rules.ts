/**
 * Farkle scoring. Pure functions over dice values, no state, no React.
 *
 * RULE SET IN USE (there are many regional variants; this is the common one).
 * If Claire's probability work assumes a different table, change it here and
 * nothing else in the app needs to move.
 *
 *   single 1 .................. 100
 *   single 5 ..................  50
 *   three 1s .................. 1000
...
export function scoreAll(values: number[]): ScoreResult {
  ...
  if (isStraight) return { points: 1500, scoringIndexes: all, usesAllDice: true };

  if (values.length === 6) {
    const pairs = c.filter((n) => n === 2).length;
    if (pairs === 3) return { points: 1500, scoringIndexes: all, usesAllDice: true };

    const triplets = c.filter((n) => n === 3).length;
    if (triplets === 2) return { points: 2500, scoringIndexes: all, usesAllDice: true };

    const hasFour = c.some((n) => n === 4);
    const hasPair = c.some((n) => n === 2);
    if (hasFour && hasPair) return { points: 1500, scoringIndexes: all, usesAllDice: true };
  }
  ...
  for (let i = 0; i < values.length; i += 1) {
    ...
    if (values[i] === 1) { points += 100; taken.add(i); scoringIndexes.push(i); }
    else if (values[i] === 5) { points += 50; taken.add(i); scoringIndexes.push(i); }
  }
